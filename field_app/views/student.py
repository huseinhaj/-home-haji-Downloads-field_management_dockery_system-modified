...
def student_confirmation_letter_pdf(request):
    """Official GoT-style confirmation letter matching TZ government placement letter format."""
    import io, os
    from reportlab.pdfgen import canvas as rl_canvas
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.units import cm
    ...
    supervisor_name = fa.assessed_by.full_name if fa.assessed_by else '—'
    deo_name        = deo.full_name if deo else 'Afisa Elimu wa Wilaya'
    ...
    kumb_no     = f"IMS.DEO/{year}/{dist_code}/{student.pk:05d}"
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # PAGE 1 — helper to draw watermarks on any page
    # ══════════════════════════════════════════════════════════════════════════
    def _draw_watermarks():
        ...
        while yp < H + 9:
            ...
            while xp < W + mw:
                c.drawString(xp, yp, micro)
                xp += mw
            yp += 9.0; ri += 1
        c.restoreState()
        c.saveState()
        c.setFont('Helvetica-Bold', 60)
        c.setFillColor(NAVY); c.setFillAlpha(0.03)
        c.translate(CX, H / 2); c.rotate(45)
    ...
    c.setLineWidth(0.5)
    c.line(LM, y, RM, y); y -= 0.5 * cm

    # ══════════════════════════════════════════════════════════════════════════
    # 3-COLUMN HEADER: Left contacts | Coat of arms | Right address
    # ══════════════════════════════════════════════════════════════════════════
    sec_top = y
    coa_w, coa_h = 70, 85
    coa_x = CX - coa_w / 2
    ...
    if coat_img_path:
        try:
            c.drawImage(coat_img_path, coa_x, sec_top - coa_h,
    ...
    # Left block
    left_lines = [
        ('bold', f'Simu ya Upepo "{school_dist.upper()}"'),
        ('mix',  'Simu: ', deo_phone),
    ...
    for bold, line in right_lines:
        ...
        if bold:
            tw = c.stringWidth(line, 'Helvetica-Bold', 8.5)
            c.setLineWidth(0.6)
            c.line(right_col, ry - 1.5, right_col + tw, ry - 1.5)
        ry -= 0.42 * cm
    ...
    dist_upper = school_dist.upper() + "."
    c.drawString(LM, y, dist_upper)
    dw = c.stringWidth(dist_upper, 'Helvetica-Bold', 10)
    c.setLineWidth(0.8)
    c.line(LM, y - 1.5, LM + dw, y - 1.5)
    y -= 1.0 * cm

    # ══════════════════════════════════════════════════════════════════════════
    # SUBJECT LINE — centred, bold, underlined
    # ══════════════════════════════════════════════════════════════════════════
    subj_text = "YAH: UTHIBITISHO WA KUKAMILISHA MAFUNZO YA UALIMU WA VITENDO"
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # BODY PARAGRAPHS — numbered, indented like GoT format
    # ══════════════════════════════════════════════════════════════════════════
    body_w  = RM - LM - 1.4 * cm
    txt_x   = LM + 1.4 * cm
    num_x   = LM + 0.5 * cm
    ...
    def draw_para(num, text, cur_y):
        ...
        lines = simpleSplit(text, 'Helvetica', 10, body_w)
        for line in lines:
    ...
    p1 = (
        f"Tafadhali rejea mafunzo ya ualimu wa vitendo (Teaching Practice) yaliyofanyika "
    ...
    p2 = (
        f"Katika tathmini iliyofanywa na msimamizi wa shule Ndg. {supervisor_name}, "
    ...
    p3 = (
        "Mafunzo yako ya ualimu wa vitendo yanakuwezesha kuendelea na taratibu nyingine "
        "za mwajiri wako kwa mujibu wa Sheria ya Utumishi wa Umma na miongozo ya Wizara "
        "ya Elimu, Sayansi na Teknolojia inayosimamia Mafunzo ya Awali ya Ualimu (ITE) nchini Tanzania."
    )
    y = draw_para("3", p3, y)

    p4 = (
        f"Aidha, unatakiwa kwenda na hati hii halisi (Original) pamoja na vyeti vyako "
        f"vya masomo ili viweze kuhakikiwa na mwajiri wako kabla ya kukamilisha taratibu za ajira. "
        f"Nambari ya uthibitisho wa hati hii ni {serial_no}."
    )
    y = draw_para("4", p4, y)

    p5 = "Nakutakia kila la kheri katika utumishi wako."
    y = draw_para("5", p5, y)

    # ══════════════════════════════════════════════════════════════════════════
    # SIGNATURE BLOCK — centred, matching GoT format
    # ══════════════════════════════════════════════════════════════════════════
    y -= 0.5 * cm
    sig_x = CX - 3 * cm   # left edge of signature block

    c.setFont('Helvetica-Oblique', 13); c.setFillColor(NAVY)
    c.drawString(sig_x, y, deo_name); y -= 0.40 * cm

    c.setStrokeColor(BLACK); c.setLineWidth(0.7)
    c.line(sig_x, y, sig_x + 6.0 * cm, y); y -= 0.44 * cm

    c.setFont('Helvetica-Bold', 10); c.setFillColor(BLACK)
    c.drawString(sig_x, y, deo_name); y -= 0.44 * cm
    c.drawString(sig_x, y, "AFISA ELIMU WA WILAYA")
    ...
    c.drawRightString(RM, footer_y + 0.22 * cm, "Hati Rasmi — IMS • Ofisi ya Afisa Elimu wa Wilaya")

    # ══════════════════════════════════════════════════════════════════════════
    # PAGE 2 — NAKALA (CC) — exactly like GoT placement letter page 2
    # ══════════════════════════════════════════════════════════════════════════
    c.showPage()
    _draw_watermarks()
    ...
    nakala_entries = [
        [
            f"Katibu Mkuu,",
            f"Ofisi ya Rais,",
            f"Menejimenti ya Utumishi wa Umma na Utawala Bora,",
            f"S.L.P. 670,",
            "DODOMA.",
        ],
        [
            f"Mkurugenzi wa Elimu ya Awali ya Ualimu,",
            f"Wizara ya Elimu, Sayansi na Teknolojia,",
            f"S.L.P. 9121,",
            "DAR ES SALAAM.",
        ],
        [
            f"Mkuu wa Shule — {school_name},",
            f"Wilaya ya {school_dist},",
            f"Mkoa wa {region_name}.",
            f"(Tafadhali mpokee na kukamilisha taratibu za Ajira yake)",
        ],
    ]

    INDENT = LM + 3.5 * cm
    for block in nakala_entries:
        ny -= 0.25 * cm
        c.setFont('Helvetica', 9.5); c.setFillColor(BLACK)
        for i, line in enumerate(block):
            bold = line.endswith('.') and not line.startswith('(')
            c.setFont('Helvetica-Bold' if (bold and i == len(block) - 2) else 'Helvetica', 9.5)
            if bold and i == len(block) - 2:
                c.drawString(INDENT, ny, line)
                tw = c.stringWidth(line, 'Helvetica-Bold', 9.5)
                c.setLineWidth(0.6)
                c.line(INDENT, ny - 1.5, INDENT + tw, ny - 1.5)
            else:
    ...
    resp['Content-Disposition'] = f'attachment; filename="Barua_Uthibitisho_{safe_name}.pdf"'
    return resp
